...
<br><br>
比如，地图上显示我们有很多来自冰岛、英国和荷兰的样本，但这并不一定代表着新冠病毒在这些地区传播得更广，而仅仅说明这些地区有更多的数据。地图上每个圆圈的大小表示的是该区域当前有多少数据可用，而不是当地疫情的真实严重程度。
...
<br><br>
这表明俄罗斯的疫情是由多次独立的输入事件引发的，而不是由病毒在单次输入之后扩散到全国各地。
<br><br>
虽然树的底部有几个样本与亚洲的样本亲缘关系最近，但大部分的俄罗斯样本是欧洲样本所属演化支的后裔。
...
然而，莫斯科的大多数样本都是与该市之外的样本最为接近，也就是说发生了多次输入事件。
<br><br>
如果我们考虑莫斯科已知的确诊病例数量，那么这些输入事件中，应该至少有一部分进一步导致了本地传播，但我们还没有足够的数据来看到这一点。
...
<br><br>
图中有7处显示，在捷克采集到的样本最有可能是欧洲其他地区所流行的病毒的后代，表明在3月1日至4月2日之间发生了新的输入事件。
<br><br>
而对于其中的5次输入事件，我们也看到了与之紧密相关的、采集时间分散的本地病例，意味着本地传播链的存在。
<br><br>
由此，我们可以总结出，捷克经历了至少7次输入事件，其中有至少5个事件很有可能进一步引发了本地传播链。
...
#### 新冠病毒曾多次输入俄罗斯；莫斯科疫情的来源和发展情况非常复杂。
<br><br>
#### 病毒曾多次输入圣彼得堡，其中一起发生在三月上旬、来源于西欧的输入事件引发了当地的一个大型本地传播链。
<br><br>
#### 土耳其曾经历反复多次的与旅行相关的输入事件，国内各地的病毒谱系已广泛、快速地混杂在一起。
<br><br>
#### 捷克存在至少5个本地传播链。
<br><br>
#### 病毒被独立地输入波兰至少11次。
<br><br>
#### 我们收到了来自匈牙利、拉脱维亚、斯洛伐克和斯洛文尼亚等地的早期样本。虽然我们还没有足够的数据来作出结论，但我们会继续密切关注当地数据，尽快提供对当地最新情况的分析。
```
...
<br><br>
[点击此处](https://github.com/nextstrain/ncov/blob/e8020ca13e420bb62e31acf9a07268bc08b11c63/narratives/sit-rep_credits.md)查看每个实验室所提供的序列详情。
<br><br>
